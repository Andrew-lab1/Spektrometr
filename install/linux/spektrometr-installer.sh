...
install_spektrometr() {
    ...
    LOCAL_SDK="../pixelinksdk-for-ubuntu2404-pc_64-v35tar"
    if [ -d "$LOCAL_SDK/lib" ] && [ -f "$LOCAL_SDK/lib/libPxLApi.so" ]; then
        print_success "Znaleziono lokalny PixeLink SDK"
        
        # Instalacja SDK
        sudo cp "$LOCAL_SDK"/lib/libPxLApi.so* /usr/local/lib/
        sudo cp "$LOCAL_SDK"/lib/libPxLApiLite.so* /usr/local/lib/
        
        # Nagłówki
        if [ -d "$LOCAL_SDK/include" ]; then
            sudo mkdir -p /usr/local/include/pixelink
            sudo cp -r "$LOCAL_SDK"/include/* /usr/local/include/pixelink/
        fi
        
        # Reguły udev
        if [ -f "$LOCAL_SDK/PixeLINK.rules" ]; then
            sudo cp "$LOCAL_SDK/PixeLINK.rules" /etc/udev/rules.d/99-pixelink.rules
            sudo udevadm control --reload-rules
            sudo udevadm trigger
        fi
        
        sudo ldconfig
        print_success "PixeLink SDK zainstalowane"
    else
        print_warning "PixeLink SDK nie znalezione - niektóre funkcje mogą nie działać"
    fi
...
#!/bin/bash
export LD_LIBRARY_PATH="/usr/local/lib:/usr/lib:/opt/spektrometr/pixelinksdk-for-ubuntu2404-pc_64-v35tar/lib:$LD_LIBRARY_PATH"
